...
public class MyBinarySearchTree {
	...
	public void insertNode(int key) {
		root=insertNodeData(root, key);
	}

	private Node insertNodeData(Node root, int key) {
		...
		if(root==null) {
			...
			if(key<root.data) {
				root.left= insertNodeData(root.left, key);
			}
			else {
				root.right =insertNodeData(root.right, key);
			}
			return root;
		}
